...
const ResearcherIdeaView = () => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      try {
        const answersResponse = await answers.getMyAnswers();
        const ideaAnswers = answersResponse.data.answers.filter(
          answer => answer.question.idea.id === id
        );
        setMyAnswers(ideaAnswers);
      } catch (error) {
  ...
  const handleSubmitAnswer = async () => {
    ...
    try {
      ...
      // Refresh data to show the new answer
      await fetchData();
  ...
  // Check which questions the user has already answered
  const answeredQuestionIds = myAnswers.map(answer => answer.question.id);
      ...
      <div className="max-w-6xl mx-auto p-6">
        ...
        <div className="grid grid-cols-3 gap-6">
          <div className="col-span-2 space-y-6">
            ...
            <div className="bg-gray-800 rounded-xl p-6 border border-gray-700">
              <h2 className="text-lg font-semibold text-white mb-4">Research Questions</h2>
                        ...
                        className={`border rounded-lg p-4 transition-all ${
                          isAnswered 
                            ...
                            : selectedQuestion?.id === question.id 
                              ...
                              : 'bg-gray-700/50 border-gray-600 hover:border-purple-500 cursor-pointer'
                        }`}
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between mt-3">
                          <span className="text-xs text-gray-400">
                            {question._count?.answers || 0} answers submitted
                          </span>
                          {isAnswered ? (
            ...
            {selectedQuestion && !answeredQuestionIds.includes(selectedQuestion.id) && (
              <div className="bg-gray-800 rounded-xl p-6 border border-purple-600">
                ...
                <div className="flex items-center justify-between mt-4">
                  <p className="text-sm text-gray-400">
                    {answerText.length} characters (min 50)
                  </p>
                ...
                <p className="text-xs text-gray-500 mt-2">
                  Minimum 50 characters required. Quality answers maintain your reputation score.
                </p>
              </div>
            )}
          </div>
              ...
              {myAnswers.length === 0 ? (
                ...
                <div className="space-y-3">
                  <p className="text-sm text-gray-400 mb-3">
                    You've answered {myAnswers.length} question{myAnswers.length > 1 ? 's' : ''}
                  </p>
                  {myAnswers.map((answer) => (
                    <div key={answer.id} className="p-3 bg-gray-700/50 rounded-lg">
                      <p className="text-sm text-white mb-1 line-clamp-2">{answer.text}</p>
                      <div className="flex items-center gap-3 text-xs text-gray-400">
                        ...
                        <span className="flex items-center gap-1">
                          Quality: {Math.round((answer.qualityScore || 0) * 100)}%
                        </span>
                      </div>
                    </div>
                  ))}
